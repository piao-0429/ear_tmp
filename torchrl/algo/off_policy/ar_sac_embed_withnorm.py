...
class ARSAC_Embed_withNorm(OffRLAlgo):
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(ARSAC_Embed_withNorm,self).__init__(**kwargs)
        ...
        self.param_optimizer = optimizer_class(
            [self.param],
            lr=self.plr,
        )
        ...
        self.sample_key = ["obs", "next_obs", "acts", "rewards", "terminals",  "task_idxs", "task_inputs", "params"]
    ...
    def update(self, batch):
            ...
            example_params = torch.Tensor(example_params)
            param = copy.deepcopy(self.param)
            params = param.expand_as(example_params).to(self.device)
            # print(params[:, 0])
            
            embedding_xs = 5 * torch.sin(params[:, 0]) * torch.cos(params[:, 1]).unsqueeze(0)
            embedding_ys = 5 * torch.sin(params[:, 0]) * torch.sin(params[:, 1]).unsqueeze(0)
            embedding_zs = 5 * torch.cos(params[:, 0]).unsqueeze(0)
            # print(embedding_xs.size())
            embeddings = torch.cat((embedding_xs, embedding_ys, embedding_zs), dim=0)
            # print(embeddings.size())
            embeddings = embeddings.T.to(self.device)
            ...
            self.param.grad = param.grad
            param_norm = torch.nn.utils.clip_grad_norm_([self.param], 10)         
            ...
            info['Training/param_norm'] = param_norm.item()
