...
class ARSAC_withNoise_v4(OffRLAlgo):
    """
    SAC_v4 for Action Representation
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.target_qf_task = copy.deepcopy(qf_task)
        ...
        self.qf1_optimizer = optimizer_class(
            (para for para in list(self.qf1.parameters()) + list(self.qf_task.parameters())),
            lr=self.qlr,
        )

        self.qf2_optimizer = optimizer_class(
            (para for para in list(self.qf2.parameters()) + list(self.qf_task.parameters())),
            lr=self.qlr,
        )
    ...
    def update(self, batch):
            ...
            embeddings4q = self.qf_task(task_inputs)
            mean4n=torch.zeros_like(embeddings4q)
            ...
            q1_pred = self.qf1([obs, actions, embeddings4q_withnoise.detach()])
            q2_pred = self.qf2([obs, actions, embeddings4q_withnoise.detach()])
            ...
            with torch.no_grad():
                ...
                target_sample_info = self.pf_action.explore(representations, embeddings_withnoise, return_log_probs=True )
                ...
                target_embeddings4q = self.target_qf_task(task_inputs)
                target_embeddings4q_withnoise = target_embeddings4q+noise
            
                
                target_q1_pred = self.target_qf1([next_obs, target_actions, target_embeddings4q_withnoise])
                target_q2_pred = self.target_qf2([next_obs, target_actions, target_embeddings4q_withnoise])
            ...
            q_new_actions = torch.min(
                self.qf1([obs, new_actions, embeddings4q_withnoise]),
                self.qf2([obs, new_actions, embeddings4q_withnoise]))
            """
            Policy Loss
            """
    ...
    def target_networks(self):
        return [
            ( self.qf_task, self.target_qf_task ),
