...
class MTSAC(TwinSACQ):
    ...
    def __init__(self, task_nums,
        ...
        self.sample_key = ["obs", "next_obs", "acts", "rewards", "terminals",  "task_idxs"]

        self.pf_flag_mt = isinstance(self.pf, policies.GuassianContPolicy)
        self.pf_flag_modular = isinstance(self.pf, policies.EmbeddingGuassianContPolicyBase) or isinstance(self.pf, policies.EmbeddingDetContPolicyBase)
        self.idx_flag = isinstance(self.pf, policies.MultiHeadGuassianContPolicy)

        self.temp_reweight = temp_reweight
        if self.pf_flag_modular or self.pf_flag_mt:
            self.sample_key.append("task_inputs")
    ...
    def update(self, batch):
        ...
        if self.idx_flag:
            ...
            if self.pf_flag_modular:
                # print(obs)
                # print(embedding_inputs)
                sample_info = self.pf.explore(obs, task_inputs,
                                            return_log_probs=True)
            
            elif self.pf_flag_mt:
                sample_info = self.pf.explore(torch.cat((obs, task_inputs),dim=-1),
                                            return_log_probs=True)
        ...
        with torch.no_grad():
            if self.idx_flag:
                ...
                if self.pf_flag_modular:
                    ...
                    target_sample_info = self.pf.explore(torch.cat((next_obs,task_inputs),dim=-1),
                                                         return_log_probs=True)
