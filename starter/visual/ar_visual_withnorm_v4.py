...
# pf_state.load_state_dict(torch.load(model_dir + "model_pf_state_18998.pth", map_location='cpu'))
# pf_task.load_state_dict(torch.load(model_dir + "model_pf_task_18998.pth", map_location='cpu'))
# pf_action.load_state_dict(torch.load(model_dir + "model_pf_action_18998.pth", map_location='cpu'))
# qf_task.load_state_dict(torch.load(model_dir + "model_qf_task_18998.pth", map_location='cpu'))
...
def save_gif_images(env_name, max_ep_len):
	...
	if params["save_embedding"]:
		...
		embed4q_dir = embed4q_dir + '/' + experiment_id + '/'
		if not os.path.exists(embed4q_dir):
			os.makedirs(embed4q_dir)
	...
	for i in range(len(task_list)):
		...
		embedding4q=qf_task.forward(task_input)
		eval_reward = 0
		sum_vel = 0
		with torch.no_grad():
