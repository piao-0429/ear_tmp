...
task_list=["vel_1", "vel_2", "vel_3", "vel_4", "vel_5", "vel_6", "vel_7", "vel_8", "vel_9", "vel_10"]
...
# pf_state.load_state_dict(torch.load(model_dir + "model_pf_state_4820.pth", map_location='cpu'))
# pf_task.load_state_dict(torch.load(model_dir + "model_pf_task_4820.pth", map_location='cpu'))
# pf_action.load_state_dict(torch.load(model_dir + "model_pf_action_4820.pth", map_location='cpu'))
...
def save_gif_images(env_name, max_ep_len):
	...
	if params["save_embedding"]:
		...
		if not os.path.exists(embed_dir):
			os.makedirs(embed_dir)	
		st_rep_dir = "state_representation"+'/'
		if not os.path.exists(st_rep_dir):
			os.makedirs(st_rep_dir)
		st_rep_dir = st_rep_dir + '/' + experiment_id + '/'
		if not os.path.exists(st_rep_dir):
			os.makedirs(st_rep_dir)
		st_rep_dir = st_rep_dir + '/' + env_name  + '/'
		if not os.path.exists(st_rep_dir):
			os.makedirs(st_rep_dir)
		st_rep_dir = st_rep_dir + '/' + str(args.seed) + '/'
		if not os.path.exists(st_rep_dir):
			os.makedirs(st_rep_dir)	
		...
		if not os.path.exists(st_dir):
			os.makedirs(st_dir)
		st_dir = st_dir + '/' + experiment_id + '/'
		if not os.path.exists(st_dir):
			os.makedirs(st_dir)
		st_dir = st_dir + '/' + env_name  + '/'
		if not os.path.exists(st_dir):
			os.makedirs(st_dir)
		st_dir = st_dir + '/' + str(args.seed) + '/'
		if not os.path.exists(st_dir):
			os.makedirs(st_dir)	
	...
	for i in range(len(task_list)):
		if params["save_embedding"]:
			...
			st_rep_csv_path = st_rep_dir + '/' + task_list[i] + ".csv"
			st_rep_file = open(st_rep_csv_path, "w")
			st_rep_writer = csv.writer(st_rep_file)
			st_csv_path = st_dir + '/' + task_list[i] + ".csv"
			st_file = open(st_csv_path, "w")
		...
		with torch.no_grad():
			for t in range(1, max_ep_len+1):
				...
				next_ob, rewards, done, info = env.step(act)
				reward = rewards[i]
				eval_reward += reward
				if params["save_velocity"]:
				...
				if t > 400 :
					...
					st_rep_writer.writerow(representation.squeeze(0).detach().cpu().numpy())
					st_writer.writerow(ob)
			...
			# Cheetah
			print(task_list[i], sum_vel/ (t - 400))
