...
def experiment(args):
    ...
    task_list=["vel_1_5_finetune_random"]
    target = 1.5
    ...
    embedding = F.normalize(torch.randn((1, embedding_shape)))
    # print(embedding)
    mean = torch.zeros(((num_sample-1) * num_best, 1, embedding_shape))
    std = torch.full(((num_sample-1) * num_best, 1, embedding_shape), 0.5)
    best_embeddings = F.normalize(torch.randn((num_best, embedding_shape))).unsqueeze(1)
